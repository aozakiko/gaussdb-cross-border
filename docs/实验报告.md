...
3. **Web 应用**：采用 Python (FastAPI) + 原生前端，完成 CRUD、过滤、Spark 批处理分析，所有 UI 控件对应后端逻辑。
...
```
前端 (HTML/CSS/JS)  <->  FastAPI (REST)  <->  GaussDB (SQLAlchemy)  <->  Spark (JDBC)
```

- **后端**：`backend/app` 使用 FastAPI + SQLAlchemy + Pydantic，提供 `/records` CRUD、`/records/analytics/spark`，并额外在根路径 `/` 集成了基于 Jinja2 的可视化控制台（表单增删改查 + 状态环形图 + 实时统计），即使在没有前端 SPA 的场景下也能直接在后端界面管理 GaussDB 数据。
- **数据库层**：`app/database.py` 动态读取 `.env` 或环境变量，可无缝切换至 GaussDB 主机；单测默认使用 SQLite。
- **大数据分析**：`app/services/spark_job.py` 通过 PySpark 连接 GaussDB，统计 `status` / `owner` 分布，封装异常处理。
...
| GET | `/records/analytics/spark` | 触发 Spark 统计 |
...
- 目前示例以 PySpark 为主，`SparkSession` 默认 `local[*]`，可通过环境变量指向 Yarn/K8s。
- 若改用 Flink，可在 `app/services` 中新增 `flink_job.py`，通过 Flink CDC 或 JDBC Connector 对 GaussDB 做实时计算，再由 FastAPI 暴露结果。
...
3. **Spark 路径**：提前在容器或主机安装 `pyspark`，配置 `SPARK_JAR_PATH` 指向 GaussDB JDBC 驱动（`opengauss-jdbc-x.y.z.jar`），点击前端“运行 Spark 汇总”按钮，观察返回 JSON。
...
本实验完成了从数据库集群搭建、容器化编排到 Web 全栈与 Spark 分析的一条龙流程，满足实验题目 7 项要求。后续可扩展：
